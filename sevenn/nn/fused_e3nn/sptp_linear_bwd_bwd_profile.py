...
def main():
    ...
    for i in range(10):
        ...
        # tensor for ours bwd
        mem_dF_dO = torch.zeros_like(out_debug)
        ...
        # mem_debug =  torch.zeros_like(out_debug)
        ...
        # original e3nn
        torch.cuda.nvtx.range_push(f"e3nn_force_bwd")
        ...
        f_in1_grad_gelu = torch.nn.functional.gelu(f_in1)
        f_in2_grad_gelu = torch.nn.functional.gelu(f_in2)
        f_weight_grad_gelu = torch.nn.functional.gelu(f_weight)
        fake_loss = f_in1_grad_gelu.sum() + f_in2_grad_gelu.sum() + f_weight_grad_gelu.sum()
        
        torch.cuda.nvtx.range_push(f"e3nn_bwd_bwd")
        fake_loss.backward()
        torch.cuda.nvtx.range_pop()
        # new_out = out_exp.grad.detach().clone()

        torch.cuda.nvtx.range_push(f"sptp_bwd_bwd")
        sptp_bwd_bwd.sptp_linear_bwd_bwd_v2_shared(f_in1, f_in2, f_weight, out_exp.grad, in1, in2, weight, 
                                           mem_dF_dO, mem_dL_dW, mem_dL_din1, mem_dL_din2,
                                           ...
                                           upath_cnt, block_batch_cnt, l_max*2+1
                                           )
