...
__global__ void fwd_sptp_lienar_kernel_v2(
    ...
    for(int path_idx=path_idx_start; path_idx < path_idx_end; path_idx++){
        ...
        for (int i =0, dldo_idx = g_t_dldo_start + path_info1.x + threadIdx.x*path_info2.x; i<path_info2.x; i++, dldo_idx++){
            out[dldo_idx] = reg_uvuv[i] * reg_w_path_norm;
        }
    }
}
...
void fwd_sptp_linear_cuda_v2(
    ...
    CHECK_CUDA_ERROR(cudaFuncSetAttribute(
        fwd_sptp_lienar_kernel_v2<float>,
        cudaFuncAttributeMaxDynamicSharedMemorySize, shared_memory_bytes));

    fwd_sptp_lienar_kernel_v2<float><<<grid, block, shared_memory_bytes>>>(
        in1.data<float>(),
        in2.data<float>(),
        weight.data<float>(),
        out.data<float>(),
