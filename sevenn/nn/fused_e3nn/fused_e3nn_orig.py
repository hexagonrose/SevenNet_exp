...
sptp_linear = load(name='sptp_linear', sources=['/home2/lsy/mdsim/fused_e3nn/fused_e3nn_kernel/sptp_linear_bwd.cpp', 
...
def _(
    ...
    sptp_linear.sptp_linear_fwd_v2_shared(in1,in2,weight, out,
...
def _(
    ...
    sptp_linear.sptp_linear_bwd_v1_shared(in1,in2,weight, grad_output.contiguous(), mem_dl_din1, mem_dl_din2, mem_dl_dw, mem_debug,
        t_in1_idxing, t_in1_ival, t_in1_related_path_idx, t_path_array1,t_path_array2,t_per_upath_fiber_start, t_path_weight, t_per_path_weight_pos, t_per_upath_fiber_array,t_unique_cg_val, upath_cnt, per_block_batch, max_ir_dim*2+1
    )
...
class fused_uvu_TP(torch.nn.Module):
    def __init__(self,
        ...
        self.l_max = max([max([x[1].l for x in i_in1]), max([x[1].l for x in i_in2]), max([x[1].l for x in i_out])])

        uvuv_tp = e3nn.o3.FullTensorProduct(i_in1,i_in2, filter_ir_out=i_out)
        uvuv_i_out = uvuv_tp.irreps_out
        tp = e3nn.o3.TensorProduct(i_in1,i_in2,i_out,inst_tuple,shared_weights=shared_weights, internal_weights=internal_weights)
        
        unique_cg, unique_cg_mat = self.extract_cg_info(uvuv_i_out, uvuv_tp.instructions)
        
        self.unique_cg_val = list(set([x.item() for x in unique_cg]))
        tp_inst_outorder = sorted(tp.instructions, key=lambda x : x.i_out)

        per_path_fiber_start, per_path_fiber_array, per_in1_ir_pathinfo = self.cgmat2fiber(tp_inst_outorder, unique_cg_mat)

        self.metadata_list = self.metadata_gen(tp.instructions, per_path_fiber_start, per_path_fiber_array, per_in1_ir_pathinfo)

    def forward(self,in1, in2, weight):
        out = torch.ops.sptp_linear.sptp_linear_fwd_v2_shared(
            in1, in2, weight , *self.metadata_list, self.per_block_batch, self.l_max, self.i_out.dim)
        return out
    ...
    def extract_cg_info(self, uvuv_i_out, instructions):
        ...
        for inst in instructions:
            ...
            partial_mat_cg = torch.zeros(self.i_in1[i].dim, self.i_in2[j].dim, uvuv_i_out[k].dim)
            # print(cg)
    ...
    def cgmat2fiber(self, tp_inst_outorder, unique_cg_mat):
        ...
        for inst in tp_inst_outorder:
            path_cg = unique_cg_mat[f"{self.i_in1[inst.i_in1][1].l}_{self.i_in2[inst.i_in2][1].l}_{self.i_out[inst.i_out][1].l}"]
    ...
    def metadata_gen(self,instructions, per_path_fiber_start, per_path_fiber_array, per_in1_ir_pathinfo):
        weight_uv_pair = []
        ...
        for inst in instructions:
            weight_uv_pair.append((self.i_in1[inst.i_in1][0], self.i_in2[inst.i_in2][0] ))
            ...
            if mul >= self.WARPSIZE:
                for i in range(mul//self.WARPSIZE):
                    ...
                    for out_ir_idx, in2_ir_idx, pw in per_in1_ir_pathinfo[in1_ir_idx]:
                        ...
                        dummy_list.append([out_slices[out_ir_idx].start + self.WARPSIZE*self.i_out[out_ir_idx].ir.dim * i,
                                        out_slices[out_ir_idx].start + self.WARPSIZE*self.i_out[out_ir_idx].ir.dim * (i+1)
                                        ])
                        dummy_list2.append([self.i_out[out_ir_idx].ir.dim,
                        ...
                        # TODO:??
                        per_path_weight_pos.append(weight_uv_pair_sorted_chunk[out2weight_order[out_ir_idx]].start + self.WARPSIZE*i)
        ...
        t_in1_related_path_idx = torch.tensor(in1_related_path_idx, dtype=torch.int32, device=self.device)

        t_path_array1 = torch.tensor(list(itertools.chain.from_iterable(path_array1)), dtype=torch.uint16, device=self.device)
        t_path_array2 = torch.tensor(list(itertools.chain.from_iterable(path_array2)), dtype=torch.uint8, device=self.device)
        t_path_weight = torch.tensor(path_weight, dtype=torch.float32, device=self.device)
        t_per_path_weight_pos = torch.tensor(per_path_weight_pos, dtype=torch.int32, device=self.device)

        t_per_upath_fiber_start = torch.tensor(per_upath_fiber_start, dtype=torch.uint16, device=self.device)
        t_per_upath_fiber_array = torch.tensor(per_upath_fiber_array, dtype=torch.uint8, device=self.device)
        t_unique_cg_val = torch.tensor(self.unique_cg_val, dtype=torch.float32, device=self.device)
        upath_cnt = len(in1_idxing)-1

        return [t_in1_idxing, t_in1_ival, t_in1_related_path_idx,t_path_array1,t_path_array2,t_per_upath_fiber_start, t_path_weight,t_per_path_weight_pos, t_per_upath_fiber_array, t_unique_cg_val, upath_cnt]
