...
__global__ void bwd_sptp_lienar_kernel_v1_shared(
    ...
    scalar_t* my_batch_shmem_start = shmem + threadIdx.y * (blockDim.x * (max_ir_dim*5 + in2_size) );
    ...
    scalar_t* my_shmem_dL_din2 = my_batch_shmem_start + blockDim.x*(max_ir_dim*4) + threadIdx.x*in2_size;
    scalar_t* shmem_scratch = my_batch_shmem_start + blockDim.x*(max_ir_dim*4 + in2_size);
    
    scalar_t* shmem_in2 = shmem + blockDim.y * (blockDim.x * (max_ir_dim*5 + in2_size)) + threadIdx.y * in2_size;
...
void bwd_sptp_linear_cuda_v1_shared(
    ...
    const int shared_memory_bytes = sizeof(float) * per_block_batch * (WARPSIZE * (max_ir_dim*5 + in2_size) + in2_size);
    ...
    CHECK_CUDA_ERROR(cudaFuncSetAttribute(
        bwd_sptp_lienar_kernel_v1_shared<float>,
        cudaFuncAttributeMaxDynamicSharedMemorySize, shared_memory_bytes));

    bwd_sptp_lienar_kernel_v1_shared<float><<<grid, block, shared_memory_bytes>>>(
        in1.data<float>(),
        in2.data<float>(),
        weight.data<float>(),
